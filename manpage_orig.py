...
class ManPage(object):
    def __init__(self, manfile, redirected_from=False):
    ...
    def add_text(self, text):
        if self.save_section_name:
            self.name_section.append(text)
            return

        buffer_to_append = None

        if self.pre_state:
            # FIXME: maybe append_text = append_text.rstrip()
            self.tree.add_content(text)
            return
        ...
        # FIXME: Probably should be moved to add_text eventually
        self.tree.add_content(text)

        if text and buffer_to_append:
            buffer_to_append.append(text)
    ...
    def start_list(self):
        args = toargs(self.line_rest)
        if len(args):
            self.tree.add_list(args[0])
    ...
    def add_section(self):
        if self.line_rest == "NAME":
            self.save_section_name = True
            return

        if self.name_section:
            self.extract_title(" ".join(self.name_section))
            self.name_section = []
            self.save_section_name = False

        self.tree.add_section(self.line_rest)

    def add_subsection(self):
        self.tree.add_subsection(self.line_rest)
    ...
    def parse_tree(self):
        ...
        return base_tpl.safe_substitute(
            ...
            content=self.tree.traverse_tree(),
        )
